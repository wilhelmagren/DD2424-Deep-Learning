...
class SnakeNet(nn.Module):
    def __init__(self):
        ...
        self.buff_size = 100
        self.memory = []
        self.l1 = nn.Linear(in_features=5*11*11, out_features=128)
        self.l2 = nn.Linear(in_features=128, out_features=64)
        self.l3 = nn.Linear(in_features=64, out_features=4)

        self.dropout = nn.Dropout(p=0.2)

    def forward(self, x):
        x = torch.reshape(x, shape=(x.shape[0], 5*11*11))
        x = F.relu(self.l1(x))
        x = F.relu(self.l2(x))
        x = self.l3(x)
        return x
...
class Snake(object):
    ...
    def serialize(self):
        bitmap = torch.zeros((5, 11, 11))
        for ydx in range(11):
            for xdx in range(11):
                if xdx == 0 or xdx == 10:
                    bitmap[0, xdx, ydx] = 1
                elif ydx == 0 or ydx == 10:
                    bitmap[0, xdx, ydx] = 1
                elif int(self.board[ydx, xdx]) == 0:
                    bitmap[4, xdx, ydx] = 1
                else:
                    bitmap[int(self.board[ydx, xdx]), xdx, ydx] = 1
...
def RLtrain(starttime, episodes, max_steps=100):
    ...
    for ep in range(episodes):
        ...
        for step in range(agent.num_iter):
            ...
            output = agent(initserialized_state.float()[None])
            ...
            statebatch = torch.cat(tuple(d[0][None] for d in batch), dim=0)
            actionbatch = torch.cat(tuple(d[1][None] for d in batch), dim=0)
            rewardbatch = torch.cat(tuple(d[2][None] for d in batch), dim=0)
            newstatebatch = torch.cat(tuple(d[3][None] for d in batch), dim=0)
            ...
            newoutputbatch = agent(newstatebatch).to(device)
            ...
            if step % 50 == 0 and step != 0:
                ...
                torch.save(agent.state_dict(), "./snake_{}_{}.pth".format(ep, step))
            if terminal.item():
                print(' |---! episode: {}, step: {},\ttime: {:.2f}s, score: {:.1f}, loss: {:.2f}'.format(ep, totstep, time.time() - starttime, score, totloss))
                scores.append(score)
                break
...
if __name__ == '__main__':
    ...
    state = Snake(10, 10)
    agent = SnakeNet()
    weights = torch.load('./snake_246_50.pth', map_location=lambda storage, loc: storage)
    agent.load_state_dict(weights)
    while state.alive():
        print(state)
        serializedstate = state.serialize()
        action = makemove(serializedstate[None], agent)
